...
def add_dns_record(router_address, auth, record):
    response = requests.put(
        f"http://{router_address}/rest/ip/dns/static", auth=auth, json=record
    )
    return response
...
def delete_dns_record(router_address, auth, record_id):
    response = requests.delete(
        f"http://{router_address}/rest/ip/dns/static/{record_id}", auth=auth
    )
    return response
...
def synchronize_dns_records(
        ...
        for address in addresses:
            ...
            if exists:
                ...
                if response.status_code in {200, 201}:
                    ...
                    if show_debug:
                        print(
                            f"DNS record successfully added: {record['name']} -> {record.get('address', record.get('cname'))}"
                        )
                elif (
                    response.status_code == 400
                    and "entry already exists" in response.text
                ):
                    if show_debug:
                        print(
                            f"DNS record already exists: {record['name']} -> {record.get('address', record.get('cname'))}"
                        )
                else:
                    if show_debug:
                        print(
                            f"Failed to add DNS record: {record['name']} -> {record.get('address', record.get('cname'))}. Status code: {response.status_code}"
                        )
                        print(response.text)
    ...
    for record_id in records_to_delete:
        ...
        if response.status_code == 204:
            ...
            if show_debug:
                print(
                    f"Failed to delete DNS record. Record ID: {record_id}. Status code: {response.status_code}"
                )
                print(response.text)
...
def main():
    ...
    parser.add_argument(
        ...
        help="display the version of mikrotik-dns-sync and exit",
    )
    parser.add_argument(
    ...
    if args.config:
        try:
            ...
            parser.error(
                f"Invalid configuration file format - missing required section: {e}"
            )
    ...
    if not all([address, username, password]):
        parser.error(
            "Router address, username, and password must be provided either through arguments, config file, or environment variables."
        )
